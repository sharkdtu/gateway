...
Gateway::Gateway()
{
	...
	hash_map<string, string> peer_to_planetlab = Configuration::instance()->get_all_planetlab_ip();
...
void Gateway::from_router_to_planetlab()
{
	...
	for(; ;)
	{
		...
		else
		{
			...
			if(!Configuration::instance()->has_peer(peer_ip))
				//ignore packet from planetlab
				continue;
			lg.dbg("receive ip packet from %s", peer_ip.c_str());

			string planetlab_ip = Configuration::instance()->get_planetlab_ip(peer_ip);
			int tcp_sockfd = planetlab_to_sockfd[planetlab_ip];
...
void Gateway::from_planetlab_to_router(string planetlab_ip)
{
	...
	for(; ;)
	{
		...
		else
		{
			...
			pair<string, string> eth = Configuration::instance()->get_eth(peer_ip);
...
void Gateway::heartbeat(string planetlab_ip)
{
	...
	for(; ;)
	{
		...
		int timeout = Configuration::instance()->get_timeout();
